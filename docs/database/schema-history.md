...
- A backup at schema `1..15` is restored via `RestoreSqliteMigrator`, which opens
  ...
  workspace.
- A backup at schema **16** is restored directly (no migration).

> The full 1→→16 path is exercised by Drift's own migration strategy; automated
> coverage currently validates a faithful v15 fixture migrating to 16 (the
> last step, which adds the doctor prescription tables). Historical pre-v15
> fixtures are marked a known limitation of the automated tests, not of the
> migration itself — the production path reuses the exact same cumulative
> `onUpgrade` logic the app has run on every upgrade since these versions
> existed.
...
- Migration coverage: `test/care_contact_migration_test.dart` asserts the schema
  version is 16 and that care-contact/doctor-visit/prescription tables and
  indexes exist; `test/restore_sqlite_migrator_test.dart` asserts v15→v16
  restore migration.
- `docs/design/design-notes.md` documents the v15→v16 additive migration and the
  Phase 10 doctor-prescriptions decisions.
