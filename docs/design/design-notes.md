...
- Persistent storage: API 29+ → `MediaStore.Downloads` (`Download/RehabTrack`,
  ...
  between layers.
- **PDF presentation redesign (Phase 11C → 11D → 11E refinement):**
  - All visual constants centralised in `ReportPdfTheme` (page geometry,
    colours, font sizes, spacing, reusable table/style helpers).
  - Branded header: "REHABTRACK" brand line + two-sided title row
    (report title left, patient name bold right-aligned) + patient phone
    and email below patient name (right-aligned, hidden when unavailable) +
    period/generated-at metadata + thin divider. Page 1 only.
  - Footer on every page: "Generated by RehabTrack" left, page N of M right.
  - **No section numbering** — hierarchy conveyed via font size, weight,
    spacing, and dividers.
  - **Section-ending dividers** after every section (same thickness/colour
    as header divider). No divider after the final section.
  - **Orphan prevention** — each section heading is wrapped with its content
    in a single `pw.Column`, so `MultiPage` cannot split the heading away
    from the initial content. If the heading + content won't fit together,
    the entire section moves to the next page.
  - Profile section: 2-column key/value grid; phone and email removed
    (now in header).
  - Medications: numbered list; ≥5 items renders in **column-major**
    2-column layout (left column fills top-to-bottom first, then right).
    Thin vertical divider between columns.
  - Measurements: **side-by-side** layout — transposed summary table
    (~35% width) + **single combined trend chart** (~65% width) for all
    components. Chart uses shared Y-axis computation (same algorithm as
    interactive Measurement Trends via `MeasurementChartAxis`).
  - Summary table: **compact two-line column headers** — short label
    (e.g. "Syst.", "Diast.", "Pulse") + unit below in smaller font.
    Unit-free centered numeric data cells. Rows: Latest / Avg / Min / Max.
  - Chart axes: **font size matches report body text** (9.5pt, same
    `ReportFonts` as all other text — no independent chart-library
    defaults). Y-axis labels rendered as integers (no ".0" decimals).
    X-axis uses **date labels** (e.g. "Jul 28", "Aug 3") with adaptive
    density (~5–8 visible labels depending on reading count).
  - Individual readings table: two-column side-by-side layout when
    reading count ≥12 (column-major split, shared headers, vertical
    divider). Compact two-line headers matching summary table.
    Header row has background decoration for clear visual separation.
  - Readings table: unit-free numeric cells, centered values.
  - Prescriptions and Lab Analyses: bordered card layout.
  - Diet: flattened hierarchy — "General Guidance" / "Foods" headings
    followed directly by category sub-headings (no redundant intermediate
    levels). Compact row-gap spacing between categories.
  - Activities: summary line + session history table with row striping.
    Zero/unknown total active duration displayed as "—" instead of "0m".
